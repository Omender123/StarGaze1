...
//        binding.txtLabelShow.setOnClickListener(this);
   ...
   private void playMedia() {
       if (events.get(position).getIntroUrl().isEmpty()) {
           ...
           videoUrl = Constants.IMG_URL1 + "" + events.get(position).getIntroUrl();
...
//                AppUtils.getTime(data.get(pos).getEndDate())
    ...
    public void isEventPaid(EventPaidCheckResp paidCheckResp) {
        ...
        Boolean aBoolean=paidCheckResp.getResult();
        String result = String.valueOf(aBoolean);
       // Toast.makeText(context, ""+paidCheckResp.getResult(), Toast.LENGTH_SHORT).show();
